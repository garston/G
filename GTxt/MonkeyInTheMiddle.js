...
GTxt.MonkeyInTheMiddle.forwardTexts = function(config) {
    GTxt.SenderMonkey.txtContacts(
        ...
        message => {
            const compressedTxt = GASton.Voice.getMessageBody(message, '');
            return GTxt.Compression.isCompressed(compressedTxt) ? compressedTxt : GASton.Voice.getMessageBody(message);
        },
        contact => {
    ...
    if(config.forwardToPhysicalPhone){
        GTxt.ReceiverMonkey.txtPhysicalPhone(this._processEmails(
            ...
            function(message){
                const txt = GASton.Voice.getMessageBody(message);
            ...
            message => GASton.Voice.getFirstNumberMentioned(message.getSubject()),
            message => GASton.Voice.getMessageBody(message),
            function(){ return 'VM'; },
            config,
            true
        )).concat(this._processEmails(
